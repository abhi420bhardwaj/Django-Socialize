 ...
 *   <li>`x` - free-spacing and line replys (aka extended)
...
/* Comment pattern: (?# )
 * Inline replys are an alternative to the line replys allowed in free-spacing mode (flag x).
 */
    add(/(?:\(\?#[^)]*\))+/,
        function (match) {
...
/* Whitespace and line replys, in free-spacing mode (aka extended mode, flag x) only.
 */
    add(/(?:\s+|#.*)+/,
        function (match) {
            // Keep tokens separated unless the following token is a quantifier
